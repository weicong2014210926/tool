...
const units: { name: string; toMs: number }[] = [
  { name: '毫秒 (ms)', toMs: 1 },
  { name: '秒 (s)', toMs: 1000 },
  { name: '分钟 (min)', toMs: 60000 },
  { name: '小时 (h)', toMs: 3600000 },
  { name: '天 (d)', toMs: 86400000 },
  { name: '周 (w)', toMs: 604800000 },
  { name: '月 (mo, 30d)', toMs: 2592000000 },
  { name: '年 (y, 365d)', toMs: 31536000000 },
];

export default function TimeUnit() {
  const [value, setValue] = useState('');
  const [from, setFrom] = useState('小时 (h)');
  const [to, setTo] = useState('分钟 (min)');
  ...
  const convert = useCallback((val: string, f: string, t: string) => {
    ...
    const result = (n * fromUnit.toMs) / toUnit.toMs;

...
const units: { name: string; toKg: number }[] = [
  { name: '毫克 (mg)', toKg: 0.000001 },
  { name: '克 (g)', toKg: 0.001 },
  { name: '千克 (kg)', toKg: 1 },
  { name: '吨 (ton)', toKg: 1000 },
  { name: '斤', toKg: 0.5 },
  { name: '两', toKg: 0.05 },
  { name: '盎司 (oz)', toKg: 0.0283495 },
  { name: '磅 (lb)', toKg: 0.453592 },
  { name: '英石 (stone)', toKg: 6.35029 },
];

export default function Weight() {
  const [value, setValue] = useState('');
  const [from, setFrom] = useState('千克 (kg)');
  const [to, setTo] = useState('磅 (lb)');
  ...
  const convert = useCallback((val: string, f: string, t: string) => {
    ...
    const result = (n * fromUnit.toKg) / toUnit.toKg;
